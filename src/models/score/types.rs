...
pub struct ScoreRow {
    ...
    #[validate(regex(
        path = "*HASH_REGEX",
        message = "HWID must contain only alphanumeric characters"
    ))]
    ...
    #[validate(regex(
        path = "*RANK_REGEX",
        message = "Rank must contain only alphanumeric characters"
    ))]
    pub rank: String,
    ...
    #[validate(regex(
        path = "*SCORE_STATUS_REGEX",
        message = "Status must contain only alphanumeric characters"
    ))]
    pub status: String,
